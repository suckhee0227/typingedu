...
// 어디서든 호출: window.dispatchEvent(new CustomEvent("open-legal", { detail: "privacy" }))
export function openLegal(key: LegalKey) {
  window.dispatchEvent(new CustomEvent("open-legal", { detail: key }));
}
...
export default function LegalModal() {
  ...
  // 전역 이벤트로 열기
  useEffect(() => {
    const onOpen = (e: Event) => setLegal((e as CustomEvent<LegalKey>).detail);
    ...
    <AnimatePresence>
      {legal && (
        <motion.div
          ...
          data-lenis-prevent
          className="fixed inset-0 z-[300] flex items-start justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm sm:p-8"
        >
          <motion.div
