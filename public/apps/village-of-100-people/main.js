...
// public/20260315042552_7a3ba845-3ae7-4656-8804-4a06e9f53b47/appHelper.ts
var AppHelper;
...
// public/20260315042552_7a3ba845-3ae7-4656-8804-4a06e9f53b47/app.ts
async function initApp() {
  const app = new VillageVillageApp();
  await app.init();
}
var ChartSegment, VillageVillageApp;
var init_app = __esm({
  "public/20260315042552_7a3ba845-3ae7-4656-8804-4a06e9f53b47/app.ts"() {
    init_appHelper();
    init_gsap();
    ChartSegment = class {
      ...
      // Updates the target value and starts GSAP animation
      update(newValue) {
    ...
    VillageVillageApp = class {
      constructor() {
        // The currently selected chart type (donut or horizontal bar)
        this.chartType = "donut";
        // The currently selected background color for the application
        this.bgColor = "#f8fafc";
        // Current region (World vs Korea)
        this.currentRegionIndex = 0;
        // Current category in selected region
        this.currentCategoryIndex = 0;
        // Current section (specific stat)
        this.currentSectionIndex = 0;
        // Array of active segments for the chart
        this.segments = [];
        // Logical resolution width
        this.logicalWidth = 1280;
        // Logical resolution height
        this.logicalHeight = 720;
        // Main rendering loop for the canvas with support for multiple chart types
        this.render = () => {
      ...
      // Strips English characters, parentheses, and translated parts to return Korean text only
      toKoreanOnly(text) {
        if (!text) return "";
      ...
      // Updates the visual state of the chart type selection buttons
      updateTypeButtons() {
        ["donut", "bar"].forEach((type) => {
          const btn = document.getElementById(`typeBtn_${type}`);
          if (btn) {
            const active = this.chartType === type;
            btn.style.backgroundColor = active ? "#3b82f6" : "white";
            btn.style.color = active ? "white" : "#64748b";
            btn.style.borderColor = active ? "#3b82f6" : "#e2e8f0";
            btn.style.boxShadow = active ? "0 4px 10px rgba(59, 130, 246, 0.2)" : "none";
          }
        });
      }
      // Draws the interactive horizontal bar chart with Korean-only labels and adjusted header text
      drawBarChart() {
        const centerX = this.logicalWidth * 0.64;
        const centerY = this.logicalHeight * 0.55;
        const barWidth = 540;
        const barHeight = 50;
        const startX = centerX - barWidth / 2;
        const startY = centerY - barHeight / 2;
        const isDarkBg = ["#164773", "#592D5F", "#000000"].includes(this.bgColor);
        const mainTextColor = isDarkBg ? "#f8fafc" : "#1e293b";
        const subTextColor = isDarkBg ? "#cbd5e1" : "#64748b";
        let currentX = startX;
        this.ctx.fillStyle = isDarkBg ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.03)";
        this.ctx.fillRect(startX - 10, startY - 10, barWidth + 20, barHeight + 20);
        this.segments.forEach((seg, idx) => {
          if (seg.value <= 0) return;
          const sliceWidth = seg.value / 100 * barWidth;
          this.ctx.fillStyle = seg.color;
          this.ctx.fillRect(currentX, startY, sliceWidth, barHeight);
          this.ctx.strokeStyle = "rgba(255,255,255,0.4)";
          this.ctx.lineWidth = 1;
          this.ctx.strokeRect(currentX, startY, sliceWidth, barHeight);
          if (sliceWidth > 40) {
            this.ctx.fillStyle = "white";
            this.ctx.font = "bold 12px sans-serif";
            this.ctx.textAlign = "center";
            this.ctx.fillText(`${Math.round(seg.value)}%`, currentX + sliceWidth / 2, startY + barHeight / 2 + 5);
          }
          const isEven = idx % 2 === 0;
          const labelY = startY + barHeight + (isEven ? 25 : 50);
          this.ctx.beginPath();
          this.ctx.moveTo(currentX + sliceWidth / 2, startY + barHeight);
          this.ctx.lineTo(currentX + sliceWidth / 2, labelY - 12);
          ...
          this.ctx.textAlign = "center";
          this.ctx.fillText(seg.label, currentX + sliceWidth / 2, labelY);
          currentX += sliceWidth;
        });
        const regionName = this.toKoreanOnly(this.appData.regions[this.currentRegionIndex].name);
        const line1 = this.toKoreanOnly(this.textData.centerInfoPrefix);
        const line2 = `${regionName} ${this.toKoreanOnly(this.textData.basisSuffix)}`;
        this.ctx.textAlign = "center";
        this.ctx.textBaseline = "middle";
        this.ctx.fillStyle = mainTextColor;
        this.ctx.font = "bold 18px sans-serif";
        this.ctx.fillText(line1, centerX, startY - 50);
        this.ctx.font = "bold 13px sans-serif";
        this.ctx.fillStyle = subTextColor;
        this.ctx.fillText(line2, centerX, startY - 30);
      }
      // Initializes the application and starts rendering
      async init() {
      ...
      // Loads external data using AppHelper
      async loadData() {
      ...
      // Draws the background with the user-selected color and a subtle grid
      drawBackground() {
        this.ctx.fillStyle = this.bgColor;
        this.ctx.fillRect(0, 0, this.logicalWidth, this.logicalHeight);
        const isDark = ["#164773", "#592D5F", "#000000"].includes(this.bgColor);
        this.ctx.strokeStyle = isDark ? "rgba(255, 255, 255, 0.08)" : "rgba(148, 163, 184, 0.1)";
        this.ctx.lineWidth = 1;
        const spacing = 40;
      ...
      // Draws the interactive donut chart with Korean-only labels and adjusted center text
      drawDonutChart() {
        const centerX = this.logicalWidth * 0.64;
        const centerY = this.logicalHeight * 0.55;
        ...
        const mainTextColor = isDarkBg ? "#f8fafc" : "#1e293b";
        const subTextColor = isDarkBg ? "#cbd5e1" : "#64748b";
        const labelTextColor = isDarkBg ? "#e2e8f0" : "#334155";
        ...
        this.segments.forEach((seg) => {
          ...
          this.ctx.fillStyle = seg.color;
          this.ctx.fill();
          this.ctx.strokeStyle = this.bgColor === "#FFFFFF" ? "#f1f5f9" : "#ffffff";
          this.ctx.lineWidth = 2;
          this.ctx.stroke();
          if (seg.value > 1) {
            const lineLen = 40;
            const lx = centerX + Math.cos(midAngle) * (outerRadius + lineLen);
            const ly = centerY + Math.sin(midAngle) * (outerRadius + lineLen);
            const isRight = Math.cos(midAngle) > 0;
            this.ctx.beginPath();
            this.ctx.moveTo(centerX + Math.cos(midAngle) * outerRadius, centerY + Math.sin(midAngle) * outerRadius);
            ...
            this.ctx.textAlign = isRight ? "left" : "right";
            const unit = this.toKoreanOnly(this.textData.unitLabel);
            this.ctx.fillText(`${seg.label}: ${Math.round(seg.value)}${unit}`, lx + (isRight ? 10 : -10), ly + 5);
          }
          startAngle = endAngle;
        });
        const regionName = this.toKoreanOnly(this.appData.regions[this.currentRegionIndex].name);
        ...
        this.ctx.fillText(line1, centerX, centerY - 15);
        this.ctx.font = "bold 14px sans-serif";
        this.ctx.fillStyle = subTextColor;
        this.ctx.fillText(line2, centerX, centerY + 15);
      }
      // Shows the entrance screen with bilingual title and improved layout
      showTitleScreen() {
        const overlay = AppHelper.createUIElement("div", "titleOverlay", {
          position: "absolute",
          top: "0",
          left: "0",
          width: "100%",
        ...
        const start = AppHelper.createUIElement(
          "button",
          "",
          {
            padding: "16px 45px",
      ...
      // Shows the guide modal
      showInstructions() {
        const box = AppHelper.createUIElement("div", "insBox", {
          position: "absolute",
          top: "10%",
          left: "25%",
          width: "50%",
        ...
        const t = AppHelper.createUIElement(
          "h2",
          "",
          {
            marginBottom: "20px",
        ...
        const c = AppHelper.createUIElement(
          "p",
          "",
          {
            lineHeight: "1.7",
          ...
          this.textData.instructionsContent
        );
        const b = AppHelper.createUIElement(
          "button",
          "",
          {
            marginTop: "30px",
        ...
        box.appendChild(c);
        box.appendChild(b);
        this.uiLayer.appendChild(box);
        gsapWithCSS.from(box, { opacity: 0, y: 50, duration: 0.8, ease: "back.out(1.7)" });
      }
      // Main UI dashboard setup with bilingual label support and flexible category layouts
      setupMainDashboard() {
        const dashboard = AppHelper.createUIElement("div", "dashboard", {
          width: "100%",
          height: "100%",
          pointerEvents: "none"
        });
        const header = AppHelper.createUIElement("div", "headerArea", {
          position: "absolute",
          top: "1%",
          left: "34%",
          width: "60%",
        ...
        this.headerTitle = AppHelper.createUIElement("div", "headerTitle", {
          color: "#0f172a",
          margin: "0",
        ...
        this.headerSubtitle = AppHelper.createUIElement("p", "", {
          fontSize: "14px",
          color: "#64748b",
          margin: "6px 0 0",
          lineHeight: "1.4",
        ...
        const typeController = AppHelper.createUIElement("div", "typeController", {
          position: "absolute",
          bottom: "3%",
          left: "48%",
        ...
        const typeLabel = AppHelper.createUIElement(
          "div",
          "",
          {
            fontSize: "10px",
          ...
          this.textData.chartTypeLabel
        );
        const typeRow = AppHelper.createUIElement("div", "", { display: "flex", gap: "8px" });
        [
          { id: "donut", label: this.textData.donutChartBtn },
          { id: "bar", label: this.textData.barChartBtn }
        ].forEach((t) => {
        ...
        typeController.appendChild(typeLabel);
        typeController.appendChild(typeRow);
        dashboard.appendChild(typeController);
        const bgController = AppHelper.createUIElement("div", "bgController", {
          position: "absolute",
          bottom: "3%",
          left: "80%",
          transform: "translateX(-50%)",
        ...
        const bgLabelText = AppHelper.createUIElement(
          "div",
          "",
          {
            fontSize: "10px",
          ...
          this.textData.bgLabel
        );
        const colorRow = AppHelper.createUIElement("div", "", { display: "flex", gap: "8px" });
        const bgColors = ["#FFFFFF", "#A3A19E", "#5DA64E", "#164773", "#592D5F", "#000000"];
        bgColors.forEach((color) => {
          const btn = AppHelper.createUIElement(
            ...
            "",
            {
              ...
              border: color === "#FFFFFF" ? "1px solid #e2e8f0" : "2px solid rgba(255,255,255,0.2)",
              cursor: "pointer",
              transition: "transform 0.2s"
            },
            "",
            [
              {
                ...
                handler: () => {
                  this.bgColor = color;
                  const isDark = ["#164773", "#592D5F", "#000000"].includes(color);
                  this.headerTitle.style.color = isDark ? "#ffffff" : "#0f172a";
                  this.headerSubtitle.style.color = isDark ? "#cbd5e1" : "#64748b";
                }
              }
            ]
          );
        ...
        bgController.appendChild(bgLabelText);
        bgController.appendChild(colorRow);
        dashboard.appendChild(bgController);
        const leftPanel = AppHelper.createUIElement("div", "leftPanel", {
          position: "absolute",
          left: "1%",
          top: "1%",
          width: "32%",
        ...
        const rLabel = AppHelper.createUIElement(
          "div",
          "",
          {
            fontSize: "11px",
          ...
          this.textData.regionSelectLabel
        );
        leftPanel.appendChild(rLabel);
        const rBtnGroup = AppHelper.createUIElement("div", "rBtnGroup", {
          display: "grid",
          gridTemplateColumns: "1fr 1fr",
          gap: "6px",
          marginBottom: "12px"
        });
        this.appData.regions.forEach((region, idx) => {
          const btn = AppHelper.createUIElement(
            "button",
            `regionBtn_${idx}`,
            {
            ...
            region.name,
            [{ event: "click", handler: () => this.selectRegion(idx) }]
          );
          rBtnGroup.appendChild(btn);
        });
        leftPanel.appendChild(rBtnGroup);
        const cLabel = AppHelper.createUIElement(
          "div",
          "",
          {
            fontSize: "11px",
          ...
          this.textData.categoryLabel
        );
        leftPanel.appendChild(cLabel);
        const cList = AppHelper.createUIElement("div", "categoriesList", {
          flex: "1",
          overflowY: "auto",
          paddingRight: "5px"
        });
        leftPanel.appendChild(cList);
        dashboard.appendChild(leftPanel);
        this.uiLayer.appendChild(dashboard);
        this.selectRegion(0);
        this.updateTypeButtons();
      }
      // Switches region data
      selectRegion(index) {
        this.currentRegionIndex = index;
        this.currentCategoryIndex = 0;
        this.currentSectionIndex = 0;
        this.appData.regions.forEach((_, i) => {
          const btn = document.getElementById(`regionBtn_${i}`);
          if (btn) {
      ...
      refreshCategories() {
        ...
        if (!list) return;
        list.innerHTML = "";
        const categories = this.appData.regions[this.currentRegionIndex].categories;
        categories.forEach((cat, cIdx) => {
          const catBox = AppHelper.createUIElement("div", "", { marginBottom: "4px" });
          const catBtn = AppHelper.createUIElement(
            "div",
            `catBtn_${cIdx}`,
            {
              padding: "8px 10px",
            ...
            cat.title,
            [{ event: "click", handler: () => this.selectCategory(cIdx) }]
          );
          const subCont = AppHelper.createUIElement("div", `subContainer_${cIdx}`, {
            paddingLeft: "8px",
            display: "none",
            marginTop: "2px",
            borderLeft: "2px solid #e2e8f0"
          });
          catBox.appendChild(catBtn);
          catBox.appendChild(subCont);
          list.appendChild(catBox);
        });
        this.selectCategory(0);
      }
      // Handles logic for expanding categories
      selectCategory(index) {
        this.currentCategoryIndex = index;
        const cats = this.appData.regions[this.currentRegionIndex].categories;
        cats.forEach((_, i) => {
          const btn = document.getElementById(`catBtn_${i}`);
          const cont = document.getElementById(`subContainer_${i}`);
          if (btn && cont) {
            const active = i === index;
            btn.style.backgroundColor = active ? "#e2e8f0" : "#f1f5f9";
            btn.style.color = active ? "#2563eb" : "#475569";
            cont.style.display = active ? "block" : "none";
            if (active) {
              cont.innerHTML = "";
              cats[i].sections.forEach((sec, sIdx) => {
                const sBtn = AppHelper.createUIElement(
                  "div",
                  `secBtn_${i}_${sIdx}`,
                  {
                    padding: "6px 10px",
        ...
        this.applySectionData(0);
      }
      // Updates the chart and UI
      applySectionData(index) {
        this.currentSectionIndex = index;
        const region = this.appData.regions[this.currentRegionIndex];
        const category = region.categories[this.currentCategoryIndex];
        const section = category.sections[index];
        this.headerTitle.innerHTML = `
      <div style="font-size: 24px; font-weight: 800; line-height: 1.2;">${category.title}</div>
      <div style="font-size: 15px; font-weight: 600; margin-top: 2px; opacity: 0.7;">${section.label}</div>
    `;
        this.headerSubtitle.innerText = this.toKoreanOnly(section.description);
        category.sections.forEach((_, i) => {
          const b = document.getElementById(`secBtn_${this.currentCategoryIndex}_${i}`);
          if (b) {
            const active = i === index;
            b.style.color = active ? "#2563eb" : "#64748b";
            b.style.fontWeight = active ? "700" : "500";
            b.style.backgroundColor = active ? "#ffffff" : "transparent";
            b.style.boxShadow = active ? "0 2px 8px rgba(37,99,235,0.06)" : "none";
          }
        });
        if (this.segments.length !== section.data.length) {
          this.segments = section.data.map((d) => new ChartSegment(this.toKoreanOnly(d.label), 0, d.color));
        }
        section.data.forEach((d, i) => {
          if (this.segments[i]) {
            this.segments[i].label = this.toKoreanOnly(d.label);
            this.segments[i].color = d.color;
            this.segments[i].update(d.value);
          } else {
            const ns = new ChartSegment(this.toKoreanOnly(d.label), 0, d.color);
            this.segments.push(ns);
            ns.update(d.value);
          }
        });
        if (this.segments.length > section.data.length) {
          this.segments.splice(section.data.length);
        }
      }
    };
  }
...
// public/20260315042552_7a3ba845-3ae7-4656-8804-4a06e9f53b47/main.ts
