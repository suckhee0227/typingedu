...
// public/20260409040325_dcb5561d-018a-4899-bf61-38f85cdbb499/appHelper.ts
var AppHelper;
  ...
  "public/20260409040325_dcb5561d-018a-4899-bf61-38f85cdbb499/appHelper.ts"() {
    ...
    AppHelper = class {
      ...
      static async loadTextData() {
        ...
        const lang = new URLSearchParams(window.location.search).get("lang") || textData.default_language;
        const texts = textData[lang] || textData[textData.default_language] || {};
        return texts;
      }
...
// public/20260409040325_dcb5561d-018a-4899-bf61-38f85cdbb499/app.ts
var import_howler, SaltParticle, ConcentrationSim, initApp;
var init_app = __esm({
  "public/20260409040325_dcb5561d-018a-4899-bf61-38f85cdbb499/app.ts"() {
    init_appHelper();
    init_gsap();
    ...
    ConcentrationSim = class {
      constructor() {
        ...
        this.targetConcentration = 0;
        this.saltAmount = 20;
        this.waterAmount = 180;
      ...
      // 모든 문항을 완료했을 때 나타나는 축하 모달을 표시합니다.
      showCompletionModal(message) {
        this.clearUI();
        const overlay = AppHelper.createUIElement("div", "comp-overlay", {
          position: "absolute",
          width: "100%",
          height: "100%",
        ...
        const box = AppHelper.createUIElement("div", "comp-box", {
          width: "40%",
          padding: "50px",
        ...
        gsapWithCSS.from(box, { scale: 0.8, opacity: 0, duration: 0.5, ease: "back.out(1.7)" });
      ...
      // 퀴즈 정답을 확인하고 다음 문항으로 넘어가는 피드백을 제공합니다.
      checkQuizAnswer(userAnswer) {
        if (isNaN(userAnswer)) return;
        const feedbackEl = document.getElementById("quiz-feedback");
        const checkBtn = document.getElementById("quiz-check-btn");
        if (!feedbackEl || !this.currentQuiz) return;
        if (userAnswer === this.currentQuiz.answer) {
          this.playSound("click");
          confetti_module_default({
            particleCount: 100,
            spread: 70,
          ...
          feedbackEl.textContent = this.textData.quizCorrect;
          feedbackEl.style.color = "#2e7d32";
          if (checkBtn) {
            checkBtn.textContent = this.quizStep === 10 ? "\uCD5C\uC885 \uC644\uB8CC" : this.textData.quizNextButton;
          ...
          feedbackEl.textContent = this.textData.quizIncorrect;
          feedbackEl.style.color = "#d32f2f";
          gsapWithCSS.fromTo(feedbackEl, { x: -5 }, { x: 5, duration: 0.05, repeat: 5, yoyo: true });
        }
      }
      // 퀴즈 모드 UI를 생성하고 진행 상황을 표시합니다.
      createQuizUI() {
        const quizContainer = AppHelper.createUIElement("div", "quiz-container", {
          position: "absolute",
          top: "6%",
          left: "51%",
          width: "28%",
          padding: "20px",
          backgroundColor: "rgba(243, 229, 245, 0.98)",
        ...
        const progress = AppHelper.createUIElement(
          "div",
          "",
          {
            fontSize: "12px",
          ...
          `${this.textData.quizProgressLabel} ${this.quizStep} / 10`
        );
        const qTitle = AppHelper.createUIElement(
          "h3",
          "",
          {
            margin: "0",
          ...
          this.textData.quizTitle
        );
        const qText = AppHelper.createUIElement(
          "p",
          "",
          {
            fontSize: "15px",
          ...
          this.currentQuiz?.qText || ""
        );
        const actionRow = AppHelper.createUIElement("div", "", {
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          gap: "10px"
        });
        const input = AppHelper.createUIElement("input", "quiz-answer-input", {
          width: "100px",
          padding: "10px",
          fontSize: "18px",
          fontWeight: "bold",
        ...
        const checkBtn = AppHelper.createUIElement(
          ...
          this.textData.quizCheckButton,
          [
            {
              event: "click",
              handler: () => this.checkQuizAnswer(parseInt(input.value))
            }
          ]
        );
        actionRow.appendChild(input);
        actionRow.appendChild(AppHelper.createUIElement("span", "", { fontWeight: "bold" }, "g"));
        actionRow.appendChild(checkBtn);
        const feedback = AppHelper.createUIElement("div", "quiz-feedback", {
          fontSize: "14px",
          fontWeight: "bold",
          textAlign: "center",
          minHeight: "20px",
        ...
        quizContainer.appendChild(actionRow);
        quizContainer.appendChild(feedback);
        this.uiLayer.appendChild(quizContainer);
        gsapWithCSS.from(quizContainer, { x: 30, opacity: 0, duration: 0.5, ease: "power2.out" });
      }
      // 퀴즈 모드를 시작하고 10개의 문제를 순차적으로 생성합니다.
      startQuiz() {
        ...
        if (this.quizStep > 10) {
          this.quizStep = 0;
          this.isQuizMode = false;
          this.showCompletionModal(this.textData.allCompleteMsg);
          return;
        }
        const problems = [
          { c: 10, w: 180, s: 20 },
        ...
        const prob = problems[this.quizStep - 1];
        const type = this.quizStep % 2 === 0 ? 2 : 1;
        if (type === 1) {
          this.currentQuiz = {
            type: 1,
            qText: `\uB18D\uB3C4\uAC00 ${prob.c}%\uC778 \uC18C\uAE08\uBB3C\uC744 \uB9CC\uB4E4\uB824\uACE0 \uD569\uB2C8\uB2E4.
\uBB3C\uC774 ${prob.w}g \uC788\uC744 \uB54C, \uD544\uC694\uD55C \uC18C\uAE08\uC758 \uC591\uC740 \uBA87 g\uC77C\uAE4C\uC694?`,
            answer: prob.s
          };
        } else {
          this.currentQuiz = {
            type: 2,
            qText: `\uC18C\uAE08 ${prob.s}g\uC744 \uB179\uC5EC\uC11C \uB18D\uB3C4\uAC00 ${prob.c}%\uC778 \uC18C\uAE08\uBB3C\uC744 \uB9CC\uB4E4\uB824\uACE0 \uD569\uB2C8\uB2E4.
\uBB3C\uC740 \uBA87 g\uC744 \uB123\uC5B4\uC57C \uD560\uAE4C\uC694?`,
            answer: prob.w
          };
        }
      ...
      // 사운드 에셋을 로드합니다.
      loadSounds() {
        this.assetList.sounds.forEach((sound) => {
          this.sounds.set(
            sound.id,
      ...
      showTitleScreen() {
        ...
        const container = AppHelper.createUIElement("div", "title-container", {
          ...
          justifyContent: "center",
          alignItems: "center",
          background: `radial-gradient(circle at center, ${this.appData.colors.background} 0%, #ffffff 100%)`,
          pointerEvents: "auto"
        });
        const title = AppHelper.createUIElement(
          "h1",
          "main-title",
          {
            fontSize: "72px",
            fontWeight: "900",
            color: this.appData.colors.uiPrimary,
            marginBottom: "20px",
            letterSpacing: "-2px"
          },
          this.textData.title
        ...
        const startBtn = AppHelper.createUIElement(
          ...
          "start-btn",
          {
            ...
            fontSize: "28px",
            fontWeight: "bold",
            backgroundColor: this.appData.colors.uiPrimary,
            color: "white",
            border: "none",
            borderRadius: "100px",
            cursor: "pointer",
            boxShadow: "0 15px 30px rgba(2, 119, 189, 0.3)",
          ...
          [
            ...
            },
            {
              ...
              handler: (e) => {
                ...
                el.style.boxShadow = "0 20px 40px rgba(2, 119, 189, 0.4)";
              }
            },
            {
              event: "mouseleave",
              handler: (e) => {
                ...
                el.style.boxShadow = "0 15px 30px rgba(2, 119, 189, 0.3)";
              }
            }
          ]
        );
      ...
      // 안내 화면을 세련된 모달 스타일로 보여줍니다.
      showInstructionScreen() {
        this.currentScreen = "instruction";
        this.clearUI();
        const overlay = AppHelper.createUIElement("div", "instr-overlay", {
          position: "absolute",
          width: "100%",
          height: "100%",
        ...
        const box = AppHelper.createUIElement("div", "instr-box", {
          width: "45%",
          padding: "50px",
        ...
        const nextBtn = AppHelper.createUIElement(
          ...
          "next-btn",
          {
            ...
            cursor: "pointer",
            fontWeight: "bold",
            boxShadow: "0 10px 20px rgba(2, 119, 189, 0.2)",
            transition: "all 0.3s ease"
          },
          "\uC2E4\uD5D8 \uC2DC\uC791\uD558\uAE30",
          [
            {
              ...
              handler: () => {
                ...
                gsapWithCSS.to(box, {
                  ...
                  opacity: 0,
                  duration: 0.3,
                  onComplete: () => this.showMainScreen()
                });
              }
            }
          ]
        ...
        overlay.appendChild(box);
        this.uiLayer.appendChild(overlay);
        gsapWithCSS.to(box, { scale: 1, opacity: 1, duration: 0.5, ease: "back.out(1.7)" });
      }
      // 메인 실험 화면을 구성하며 모드 전환 시 진행 단계를 초기화합니다.
      showMainScreen() {
        this.currentScreen = "main";
        this.clearUI();
        this.updateConcentration();
        const leftContainer = AppHelper.createUIElement("div", "left-container", {
          position: "absolute",
          top: "4%",
          left: "2.5%",
          width: "30%",
        ...
        const sidePanel = AppHelper.createUIElement("div", "side-panel", {
          width: "100%",
          padding: "25px 20px",
        ...
        const panelTitle = AppHelper.createUIElement(
          "h2",
          "",
          {
            fontSize: "20px",
            ...
            color: this.appData.colors.uiPrimary,
            textAlign: "center",
            borderBottom: `2px solid ${this.appData.colors.uiPrimary}22`,
            paddingBottom: "10px",
            fontWeight: "900"
          },
          "\uC2E4\uD5D8 \uB3C4\uAD6C\uD568"
        );
        const saltControl = this.createModernSlider(
          this.textData.saltLabel,
          0,
          300,
          this.saltAmount,
          (val) => {
        ...
        const waterControl = this.createModernSlider(
          this.textData.waterLabel,
          100,
          900,
          this.waterAmount,
          (val) => {
        ...
        sidePanel.appendChild(saltControl);
        sidePanel.appendChild(waterControl);
        const infoPanel = AppHelper.createUIElement("div", "info-panel", {
          width: "100%",
          flex: "1",
          padding: "25px 20px",
          backgroundColor: "rgba(255, 255, 255, 0.92)",
        ...
        const expText = AppHelper.createUIElement("div", "explanation-text", {
          fontSize: "15px",
          lineHeight: "1.5",
          color: "#455a64",
          marginBottom: "15px"
        });
        const formulaContainer = AppHelper.createUIElement("div", "formula-container", {
          padding: "15px 10px",
        ...
        const formulaLabel = AppHelper.createUIElement(
          "p",
          "",
          {
            fontSize: "12px",
        ...
        const formulaText = AppHelper.createUIElement("div", "formula-text", {
          fontSize: "15px",
          fontWeight: "bold",
          color: "#33691e",
        ...
        const concResult = AppHelper.createUIElement("div", "concentration-result", {
          fontSize: "26px",
          fontWeight: "900",
          color: this.appData.colors.uiPrimary,
          marginTop: "auto",
        ...
        infoPanel.appendChild(expText);
        infoPanel.appendChild(formulaContainer);
        infoPanel.appendChild(concResult);
        leftContainer.appendChild(sidePanel);
        leftContainer.appendChild(infoPanel);
        const modeBtn = AppHelper.createUIElement(
          "button",
          "mode-toggle-btn",
          {
            position: "absolute",
            ...
            padding: "14px",
            fontSize: "16px",
            backgroundColor: this.isMissionMode ? "#90a4ae" : this.appData.colors.uiAccent,
            color: "white",
            border: "none",
            borderRadius: "15px",
            cursor: "pointer",
          ...
          this.isMissionMode ? "\u{1F3E0} \uC77C\uBC18" : `\u{1F3AF} ${this.textData.missionButton}`,
          [
            {
              event: "click",
              handler: () => {
        ...
        const quizBtn = AppHelper.createUIElement(
          "button",
          "quiz-toggle-btn",
          {
            position: "absolute",
            ...
            padding: "14px",
            fontSize: "16px",
            backgroundColor: this.isQuizMode ? "#90a4ae" : "#673ab7",
            color: "white",
            border: "none",
            borderRadius: "15px",
            cursor: "pointer",
          ...
          this.isQuizMode ? "\u{1F3E0} \uC77C\uBC18" : `\u{1F4DD} ${this.textData.quizButton}`,
          [
            {
              event: "click",
              handler: () => {
        ...
        this.uiLayer.appendChild(leftContainer);
        this.uiLayer.appendChild(modeBtn);
        this.uiLayer.appendChild(quizBtn);
        if (this.isMissionMode) {
      ...
      // 미션 모드를 시작하고 10개의 목표 농도를 순차적으로 제시합니다.
      startMission() {
        this.isMissionMode = true;
        this.isQuizMode = false;
        this.missionStep++;
        if (this.missionStep > 10) {
          this.missionStep = 0;
          this.isMissionMode = false;
          this.showCompletionModal(this.textData.allCompleteMsg);
          return;
        }
        this.targetConcentration = Math.floor(Math.random() * 20) + 5;
        this.showMainScreen();
      }
      // 챌린지 모드 UI를 생성하고 진행 상황을 표시합니다.
      createMissionUI() {
        const missionContainer = AppHelper.createUIElement("div", "mission-container", {
          position: "absolute",
          top: "6%",
          left: "51%",
          width: "27%",
          padding: "15px",
          backgroundColor: "rgba(255, 252, 231, 0.98)",
          backdropFilter: "blur(10px)",
          border: `2px solid ${this.appData.colors.uiAccent}`,
          borderRadius: "20px",
          display: "flex",
          flexDirection: "column",
          gap: "10px",
        ...
        const progress = AppHelper.createUIElement(
          ...
          `${this.textData.missionProgressLabel} ${this.missionStep} / 10`
        );
        const mTitle = AppHelper.createUIElement(
          "h3",
          "",
          {
            margin: "0",
        ...
        const targetInfo = AppHelper.createUIElement("div", "", {
          display: "flex",
          flexDirection: "column"
        });
        const targetLabel = AppHelper.createUIElement(
          "span",
          "",
          {
            fontSize: "10px",
        ...
        const targetVal = AppHelper.createUIElement(
          "span",
          "goal-value",
          {
            fontSize: "18px",
        ...
        targetInfo.appendChild(targetVal);
        const checkBtn = AppHelper.createUIElement(
          "button",
          "check-btn",
          {
            padding: "8px 15px",
            fontSize: "13px",
            backgroundColor: this.appData.colors.uiAccent,
            color: "white",
            border: "none",
            borderRadius: "10px",
            cursor: "pointer",
        ...
        actionRow.appendChild(targetInfo);
        actionRow.appendChild(checkBtn);
        const feedbackText = AppHelper.createUIElement("div", "mission-feedback", {
          fontSize: "13px",
          fontWeight: "bold",
          color: "#6d4c41",
          textAlign: "center",
        ...
        missionContainer.appendChild(progress);
        missionContainer.appendChild(mTitle);
        missionContainer.appendChild(actionRow);
        missionContainer.appendChild(feedbackText);
        this.uiLayer.appendChild(missionContainer);
        gsapWithCSS.from(missionContainer, { y: -20, opacity: 0, duration: 0.4, ease: "back.out(1.2)" });
      }
      // 미션 성공 여부를 체크하고 다음 챌린지로 넘어가는 피드백을 제공합니다.
      checkMissionSuccess() {
        const diff = Math.abs(this.concentration - this.targetConcentration);
        ...
        if (diff < 0.01) {
          ...
          confetti_module_default({
            ...
            spread: 80,
            origin: { y: 0.6 },
            colors: ["#ff9800", "#ffffff", "#4caf50", "#2196f3"]
          });
          feedbackEl.textContent = `\u{1F3AF} \uC644\uBCBD\uD55C \uB18D\uB3C4\uC785\uB2C8\uB2E4! (${this.concentration.toFixed(2)}%)`;
          feedbackEl.style.color = "#2e7d32";
          if (checkBtn) {
            checkBtn.textContent = this.missionStep === 10 ? "\uCD5C\uC885 \uC644\uB8CC" : "\uB2E4\uC74C \uBBF8\uC158 \uB3C4\uC804!";
            checkBtn.style.backgroundColor = "#4caf50";
            checkBtn.style.boxShadow = "0 8px 15px rgba(76, 175, 80, 0.3)";
            const newBtn = checkBtn.cloneNode(true);
            checkBtn.parentNode?.replaceChild(newBtn, checkBtn);
            newBtn.addEventListener("click", () => {
              this.playSound("click");
          ...
          const hint = this.concentration > this.targetConcentration ? "\uC870\uAE08 \uB354 \uC5F0\uD558\uAC8C(\uBB3C\uC744 \uCD94\uAC00) \uD574\uBCFC\uAE4C\uC694?" : "\uC870\uAE08 \uB354 \uC9C4\uD558\uAC8C(\uC18C\uAE08\uC744 \uCD94\uAC00) \uD574\uBCFC\uAE4C\uC694?";
          feedbackEl.textContent = `\uD604\uC7AC ${this.concentration.toFixed(2)}%... ${hint}`;
          feedbackEl.style.color = "#d32f2f";
          gsapWithCSS.fromTo(feedbackEl, { x: -5 }, { x: 5, duration: 0.05, repeat: 5, yoyo: true });
        }
      }
      // 세련된 디자인과 직접 숫자 입력이 가능한 슬라이더를 생성합니다.
      createModernSlider(label, min, max, initial, onChange, unit) {
        ...
        const inputWrapper = AppHelper.createUIElement("div", "", {
          display: "flex",
          alignItems: "center",
          backgroundColor: `${this.appData.colors.uiPrimary}11`,
          padding: "4px 8px",
          borderRadius: "8px"
        });
        const valInput = AppHelper.createUIElement("input", "", {
          fontSize: "19px",
          fontWeight: "900",
          color: this.appData.colors.uiPrimary,
        ...
        valInput.value = initial.toString();
        const unitEl = AppHelper.createUIElement(
          "span",
          "",
          {
            fontSize: "17px",
        ...
        const controlArea = AppHelper.createUIElement("div", "", {
          display: "flex",
          alignItems: "center",
          gap: "10px"
        });
        const createBtn = (symbol, delta) => {
            ...
            [
              {
                ...
                handler: () => {
                  this.playSound("click");
                  const newVal = Math.max(min, Math.min(max, parseInt(input.value) + delta));
                  input.value = newVal.toString();
                  valInput.value = newVal.toString();
                  onChange(newVal);
                }
              }
            ]
        ...
        const minusBtn = createBtn("-", -1);
        const plusBtn = createBtn("+", 1);
        const input = AppHelper.createUIElement("input", "", {
          flex: "1",
          cursor: "pointer",
          accentColor: this.appData.colors.uiPrimary,
          height: "6px"
        });
        input.type = "range";
        ...
        controlArea.appendChild(input);
        controlArea.appendChild(plusBtn);
        container.appendChild(header);
        container.appendChild(controlArea);
        return container;
      }
      // 농도를 업데이트하고 입자들의 위치를 수위에 맞춰 동적으로 조정합니다.
      updateConcentration() {
        const total = this.saltAmount + this.waterAmount;
        if (total === 0) {
          this.concentration = 0;
        } else {
          this.concentration = this.saltAmount / total * 100;
        }
        const targetCount = Math.floor(this.saltAmount * 1.5);
        const centerX = this.appData.canvasWidth * 0.6625;
        const centerY = this.appData.canvasHeight * 0.68;
        const bH = 380;
        const bottomY = centerY + bH / 2 - 15;
        const scaleHeight = bH * 0.85;
        const waterLevelHeight = Math.min(this.waterAmount / 1e3 * scaleHeight, scaleHeight);
        const topY = bottomY - waterLevelHeight;
        if (this.particles.length < targetCount) {
          for (let i = this.particles.length; i < targetCount; i++) {
            const px2 = centerX + (Math.random() - 0.5) * 240;
            const py = topY + Math.random() * (bottomY - topY);
            this.particles.push(new SaltParticle(px2, py));
          }
        } else {
          this.particles.splice(targetCount);
        }
      }
      // 화면의 텍스트와 공식을 업데이트합니다.
      updateUI() {
        const expTextEl = document.getElementById("explanation-text");
        const resultEl = document.getElementById("concentration-result");
        const formulaEl = document.getElementById("formula-text");
        if (expTextEl) {
          const total = this.saltAmount + this.waterAmount;
          expTextEl.replaceChildren();
          const line1 = AppHelper.createUIElement("div", "", {});
          line1.appendChild(AppHelper.createUIElement("span", "", {}, "\uBB3C "));
          line1.appendChild(
            AppHelper.createUIElement("span", "", { fontWeight: "bold", color: "#0288d1" }, `${this.waterAmount}g`)
          );
          line1.appendChild(AppHelper.createUIElement("span", "", {}, "\uACFC \uC18C\uAE08 "));
          line1.appendChild(
            AppHelper.createUIElement("span", "", { fontWeight: "bold", color: "#795548" }, `${this.saltAmount}g`)
          );
          line1.appendChild(AppHelper.createUIElement("span", "", {}, "\uC758 \uD63C\uD569\uBB3C\uC5D0\uC11C"));
          const principleBox = AppHelper.createUIElement(
            "div",
            "",
            {
              marginTop: "10px",
            ...
            this.textData.concentrationPrinciple
          );
          expTextEl.appendChild(line1);
          expTextEl.appendChild(principleBox);
        }
        if (formulaEl) {
          formulaEl.replaceChildren();
          const fracContainer = AppHelper.createUIElement("div", "", {
            display: "inline-flex",
            flexDirection: "column",
            alignItems: "center",
          ...
          const numerator = AppHelper.createUIElement(
            "div",
            "",
            {
              padding: "0 5px 2px 5px",
              borderBottom: "1.5px solid #33691e",
              fontSize: "16px"
            },
            `${this.saltAmount}`
          );
          const denominator = AppHelper.createUIElement(
            "div",
            "",
            {
            ...
            `${this.saltAmount + this.waterAmount}`
          );
          fracContainer.appendChild(numerator);
          fracContainer.appendChild(denominator);
          formulaEl.appendChild(document.createTextNode("( "));
          formulaEl.appendChild(fracContainer);
          formulaEl.appendChild(document.createTextNode(" ) \xD7 100 = "));
          formulaEl.appendChild(
            AppHelper.createUIElement(
              "span",
              "",
              { color: "#1b5e20", fontWeight: "900", marginLeft: "4px" },
              `${this.concentration.toFixed(2)}%`
            )
          );
        }
        if (resultEl) {
          ...
          resultEl.appendChild(
            AppHelper.createUIElement(
              "span",
              "",
              { fontSize: "18px", color: "#666", marginRight: "10px", fontWeight: "bold" },
              this.textData.resultLabel
            )
          );
          resultEl.appendChild(AppHelper.createUIElement("span", "", {}, `${this.concentration.toFixed(2)}%`));
        }
      }
      // 실험실 분위기의 배경을 그립니다.
      drawBackground() {
        const { width: w, height: h } = this.canvas;
        const wallGrad = this.ctx.createLinearGradient(0, 0, 0, h);
        ...
        this.ctx.fillRect(0, h * 0.75, w, 4);
        ...
        for (let x = 0; x < w; x += tileSize) {
          this.ctx.beginPath();
          this.ctx.moveTo(x, 0);
          this.ctx.lineTo(x, h * 0.75);
          this.ctx.stroke();
        }
        for (let y = 0; y < h * 0.75; y += tileSize) {
          this.ctx.beginPath();
          this.ctx.moveTo(0, y);
          this.ctx.lineTo(w, y);
          this.ctx.stroke();
        ...
        this.ctx.beginPath();
        this.ctx.arc(w * 0.9, h * 0.15, 80, 0, Math.PI * 2);
        this.ctx.fill();
        this.ctx.globalAlpha = 1;
      }
      // 시뮬레이션 요소를 캔버스에 그립니다. 비커 내부 마스킹 및 동적 수위 반영.
      drawSimulation() {
        const centerX = this.appData.canvasWidth * 0.6625;
        const centerY = this.appData.canvasHeight * 0.68;
        const bW = 280;
        const bH = 380;
        const maxCapacity = 1e3;
        const scaleHeight = bH * 0.85;
        const waterLevelHeight = Math.min(this.waterAmount / maxCapacity * scaleHeight, scaleHeight);
        const bottomY = centerY + bH / 2 - 15;
        const topY = bottomY - waterLevelHeight;
        this.ctx.fillStyle = "rgba(255, 255, 255, 0.4)";
        this.ctx.beginPath();
        this.ctx.roundRect(centerX - bW / 2, centerY - bH / 2, bW, bH, [0, 0, 25, 25]);
        this.ctx.fill();
        const waterPath = new Path2D();
        waterPath.moveTo(centerX - bW / 2 + 10, bottomY);
        waterPath.lineTo(centerX + bW / 2 - 10, bottomY);
        waterPath.lineTo(centerX + bW / 2 - 10, topY);
        for (let x = centerX + bW / 2 - 10; x >= centerX - bW / 2 + 10; x -= 5) {
          const waveY = topY + Math.sin(x * 0.04 + this.waveTime * 3) * 3;
          waterPath.lineTo(x, waveY);
        }
        waterPath.closePath();
        const waterGrad = this.ctx.createLinearGradient(centerX, topY, centerX, bottomY);
        waterGrad.addColorStop(0, this.appData.colors.water);
        waterGrad.addColorStop(1, "#0277bd");
        this.ctx.save();
        this.ctx.globalAlpha = 0.6;
        this.ctx.fillStyle = waterGrad;
        this.ctx.fill(waterPath);
        this.ctx.restore();
        this.ctx.save();
        this.ctx.clip(waterPath);
        this.ctx.fillStyle = this.appData.colors.salt;
        this.particles.forEach((p) => {
          p.update(centerX - bW / 2 + 15, centerX + bW / 2 - 15, topY, bottomY - 5);
          this.ctx.beginPath();
          this.ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
          this.ctx.fill();
        });
        this.ctx.restore();
        this.ctx.strokeStyle = this.appData.colors.glass;
        this.ctx.lineWidth = 10;
        this.ctx.lineJoin = "round";
        this.ctx.beginPath();
        this.ctx.moveTo(centerX - bW / 2, centerY - bH / 2);
        this.ctx.lineTo(centerX - bW / 2, bottomY - 10);
        this.ctx.quadraticCurveTo(centerX - bW / 2, bottomY + 10, centerX - bW / 2 + 20, bottomY + 10);
        this.ctx.lineTo(centerX + bW / 2 - 20, bottomY + 10);
        this.ctx.quadraticCurveTo(centerX + bW / 2, bottomY + 10, centerX + bW / 2, bottomY - 10);
        this.ctx.lineTo(centerX + bW / 2, centerY - bH / 2);
        this.ctx.stroke();
        this.ctx.strokeStyle = "rgba(255, 255, 255, 0.6)";
        this.ctx.lineWidth = 4;
        this.ctx.beginPath();
        this.ctx.moveTo(centerX - bW / 2 + 25, centerY - bH / 2 + 20);
        this.ctx.lineTo(centerX - bW / 2 + 25, bottomY - 20);
        this.ctx.stroke();
        ...
        for (let i = 1; i <= 10; i++) {
          const ly = bottomY - scaleHeight / 10 * i;
          this.ctx.beginPath();
          this.ctx.moveTo(centerX + bW / 2 - 30, ly);
          this.ctx.lineTo(centerX + bW / 2 - 10, ly);
          this.ctx.stroke();
          if (i % 2 === 0) {
            this.ctx.fillText(`${i * 100}ml`, centerX + bW / 2 + 10, ly + 4);
          }
        }
      }
      // 애니메이션 루프
    ...
    initApp = async () => {
      const app = new ConcentrationSim();
      await app.init();
    };
  }
});

// public/20260409040325_dcb5561d-018a-4899-bf61-38f85cdbb499/main.ts
